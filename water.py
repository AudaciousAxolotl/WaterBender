...
class Point:
    """
    Draws the points that the voronoi makes it graph around.
    """
...
class Graph:
    ...
    def initialize_centers(self):
        ...
        for point in voronoi.points:
            centers.append(Center(Point(point[0] * self.MAP_SIZE, point[1] * self.MAP_SIZE), len(centers)))

        for vertex in voronoi.vertices:
            corners.append(Corner(Point(vertex[0] * self.MAP_SIZE, vertex[1] * self.MAP_SIZE), len(corners)))
