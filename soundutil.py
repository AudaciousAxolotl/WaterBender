...
class SoundPlayer:
	...
	def pause(self):
		...
		if self.buffer_player.state == libaudioverse.NodeStates.playing:
			self.buffer_player.state = libaudioverse.NodeStates.paused
		
	def resume(self):
		"""Resumes the playback of a file."""
		if self.buffer_player.state == libaudioverse.NodeStates.paused:
...
class Listener:
	...
	def move(self, vel):
		"""Moves the listener."""
		self.position += vel
		self.environment.position = Float3(*tuple(self.position))